...
beforeEach(async () => {
  sandbox = await mkdtemp(join(tmpdir(), "loopeng-e2e-"));
  loopengHomeDir = join(sandbox, "loopeng-home");
  ...
  process.env.LOOPENG_HOME = loopengHomeDir;
});
...
describe("loopeng end-to-end pipeline", () => {
  it("digests transcripts, proposes a loop, bundles it, installs it, and uninstalls cleanly", async () => {
    ...
    const digestDir = join(loopengHomeDir, "digests");
    ...
    const bundle = await generateBundle(candidate, {
      runner: bundleRunner,
      bundlesDir: join(loopengHomeDir, "bundles"),
      now: NOW
    });
    ...
    const plistPath = join(launchAgentsDir, "com.loopeng.ci-watch.plist");
