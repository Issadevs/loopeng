...
beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), "loopeng-watch-home-"));
  claudeDir = await mkdtemp(join(tmpdir(), "loopeng-watch-claude-"));
  codexDir = await mkdtemp(join(tmpdir(), "loopeng-watch-codex-"));
...
describe("watcher tick", () => {
  it("digests new claude-code and codex transcript files", async () => {
    ...
    expect(spawned).toEqual([["loopeng", "companion"]]);
