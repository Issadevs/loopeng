...
beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), "loopeng-shell-"));
  userHome = await mkdtemp(join(tmpdir(), "loopeng-shell-home-"));
...
function plistPath(): string {
  return join(userHome, "Library", "LaunchAgents", "com.loopeng.daemon.plist");
}
...
describe("assembleData", () => {
  ...
  it("assembles proposals, loops, spend, events, and daemon state from disk", async () => {
    ...
    writeJsonAtomic(join(loopengHome(), "log", "spend.json"), { [TODAY]: 1234 });
...
describe("dispatchEffect", () => {
  ...
  it("approve generates a bundle and installs the loop", async () => {
    saveProposal({ candidate: candidate({ id: "a-1" }), status: "pending", createdAt: NOW });

    let generated = false;
    const flash = await dispatchEffect(
      makeDeps({
        runner: async (prompt: string) => {
          if (prompt.includes("MAKER")) {
            generated = true;
            return `## Responsibility\nhandle it\n## Trigger & cadence\ndaily\n## Procedure\ndo it\n## Verification\ncheck it\n## Convergence\n3 tries\n## Escalation\nescalate\n\n\`\`\`json\n{"kind": "manual", "tool": "claude-code"}\n\`\`\``;
          }
    ...
    expect(flash).toContain("a-1");
    expect(getProposal("a-1")?.status).toBe("approved");
  });

  it("returns the error message when the runner throws", async () => {
    seedDigests();
    const deps = makeDeps({
      runner: async () => {
    ...
    const flash = await dispatchEffect(deps, { type: "scan" });
    expect(flash).toBe("scan failed: scan engine error: LLM unavailable");
  });
});
