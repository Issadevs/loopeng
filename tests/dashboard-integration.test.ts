...
import { addToRegistry, getProposal, loopengHome, saveProposal, writeJsonAtomic } from "../src/state.js";
...
beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), "loopeng-int-"));
  userHome = await mkdtemp(join(tmpdir(), "loopeng-int-home-"));
...
describe("dashboard integration", () => {
  ...
  it("runs a scan cycle that writes a new proposal and surfaces it in the inbox", async () => {
    ...
    expect(existsSync(join(loopengHome(), "proposals", "cand-scanned.json"))).toBe(true);
