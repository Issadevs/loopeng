...
describe("installClaudeCodeLoop - schedule", () => {
  it("writes a plist, loads it, and records every touched path in the manifest", async () => {
    ...
    const plistPath = join(ctx.launchAgentsDir, "com.loopeng.abc123.plist");

    expect(existsSync(plistPath)).toBe(true);
    const plist = await readFile(plistPath, "utf8");
    expect(plist).toContain("com.loopeng.abc123");
  ...
  it("shell-quotes a loop.md path containing a space", async () => {
    ...
    await installClaudeCodeLoop(bundleDir, ctx);
    const plistPath = join(ctx.launchAgentsDir, "com.loopeng.spc1.plist");
    const plist = await readFile(plistPath, "utf8");
...
describe("installClaudeCodeLoop - hook", () => {
  ...
  it("appends additively, preserving existing content; uninstall is byte-clean", async () => {
    ...
    expect(afterInstall.hooks.PreToolUse[1].hooks[0].command).toContain("# loopeng:hook99");
  ...
  it("removes only the loopeng command from a shared entry, sparing user commands", async () => {
    ...
    const loopengCommand = {
      type: "command",
      command: `sh -c 'claude -p "$(cat x)"' # loopeng:${loopId}`
    };
    const sharedSettings = {
      hooks: {
        PreToolUse: [{ matcher: "*", hooks: [userCommand, loopengCommand] }]
      }
    };
    ...
    expect(dumped).not.toContain(`# loopeng:${loopId}`);
  });
...
describe("installCodexLoop", () => {
  it("round-trips a schedule install", async () => {
    ...
    const plistPath = join(ctx.launchAgentsDir, "com.loopeng.cdx1.plist");
