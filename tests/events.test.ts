...
import { dismissAction, snoozeAction } from "../src/actions.js";
import { type CliDeps } from "../src/cli.js";
import { loopengHome, saveProposal } from "../src/state.js";
...
function eventsPath(): string {
  return join(loopengHome(), "log", "events.jsonl");
}

beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), "loopeng-events-"));
  userHome = await mkdtemp(join(tmpdir(), "loopeng-events-home-"));
