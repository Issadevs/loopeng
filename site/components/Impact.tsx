...
// loopeng result (amber, animates in). Direction conveys "manual worse, loopeng
...
export default function Impact() {
  const { impact: impactContent, sectionLabels, manualLabel, loopengLabel } = useContent();
  ...
  return (
    <section className="mx-auto w-full max-w-4xl px-6 py-24">
      <SectionLabel>{sectionLabels.impact}</SectionLabel>

      <h2 className="mt-4 font-serif text-4xl leading-[1.1] sm:text-5xl">
        {impactContent.heading}
        <br />
        <span className="text-amber">{impactContent.headingAccent}</span>
      </h2>

      <div ref={ref} className="mt-12 space-y-8">
        {impactContent.bars.map((bar, i) => {
          ...
          return (
            <div key={bar.label}>
              ...
              <div className="mt-2 space-y-2">
                ...
                {/* after — loopeng result (amber, animates width on scroll-in) */}
                <div className="relative h-9 overflow-hidden rounded-md border border-border bg-inset">
                  <div
      ...
      <p className="mt-12 text-center text-lg text-text sm:text-xl">
        {impactContent.kicker}
      </p>
      <p className="mt-3 text-center text-xs text-dim">{impactContent.footnote}</p>
    </section>
  );
}
