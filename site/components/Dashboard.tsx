...
export default function Dashboard() {
  const { dashboard: dashboardContent, sectionLabels } = useContent();
  return (
    <section className="mx-auto w-full max-w-5xl px-6 py-20 sm:py-28">
      <div className="flex flex-col gap-4">
        <SectionLabel>{sectionLabels.dashboard}</SectionLabel>
      ...
      <div className="mt-10 flex flex-col gap-4">
        <p className="font-mono text-sm text-green">{dashboardContent.command}</p>
        <div className="overflow-hidden rounded-lg border border-border bg-raised p-2 sm:p-3">
          <Image
            src="/dashboard.png"
            width={1676}
            height={672}
            alt="loopEng terminal dashboard showing inbox, loops, and activity panels"
      ...
      <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-3">
        {dashboardContent.bullets.map((bullet) => (
      ...
      <p className="mt-8 text-sm text-muted">{dashboardContent.subline}</p>
    </section>
  );
}
